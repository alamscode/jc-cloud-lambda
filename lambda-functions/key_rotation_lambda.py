...
def filter_users(IAM, user_name, start_time, end_time, exclude_list):
    ...
    for keys in keydetails['AccessKeyMetadata']:
        if (time_diff(keys['CreateDate'])) >= start_time and (time_diff(keys['CreateDate'])) <= end_time:
            user = {}
...
def lambda_handler(event, context):
    ...
    exclude_list = os.environ['exclude_list'].replace(" ", "")
    exclude_list = exclude_list.split(',')    
    deletion = os.environ['deletion']    
    expiration = int(os.environ['expiration'])
    warning_period = int(os.environ['warning_period'])
    warning = int(expiration) - warning_period
    receipient_addresses = os.environ['receipient_addresses'].split(',')    
